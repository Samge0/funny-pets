...
- **吞噬系统**：战斗胜利有机会夺取对手的技能与外观部件——空槽直接长出，已有部件可选叠加（贴身多长一件，上限 8）或替换；花纹/眼睛为单值部件自动换上；图鉴小图实时反映战利品。
- **灵魂对话（可选 AI）**：每只精灵有独立人格（性格四维/口癖/喜好/信念）与三层记忆；配置 LLM 后可与它聊天（记忆自动沉淀压缩），战斗中敌我双方都会用流式台词喊话——野生精灵自带"领地被闯入"的野性人设。
- **分享挑战**：宠物详情页一键生成分享链接（纯前端，无后端）——好友打开即可 360° 观赏 3D 精灵（点击会跳），还能用自己的精灵发起挑战（切磋不触发吞噬/捕捉，结果不影响分享者存档）。
- **赠送宠物**：详情页生成赠送链接，好友打开即可领取一只**克隆**——你的原宠不丢失。克隆的灵魂与 AI 聊天用领取者自己的配置重新开始，API Key 绝不随链接外传。
- **存档**：localStorage 本地保存 + 一键导出/导入 JSON 存档文件（带校验，防脏数据）。
- **AI 生成（可选）**：设置页配置任意 OpenAI 兼容接口（Base URL / Model / API Key）后，每次刷新精灵由大模型生成名字、属性组合与图鉴描述；12 秒超时 + 自动重试 + 容错解析，关闭或持续失败时才降级本地随机。API Key 只存在你自己的浏览器里。
- **18 种语言** 🌍：界面、战报、实体名（属性/技能/地图/稀有度/性格/部件）与全部 LLM 输出跟随设置页选择的语言——默认跟随浏览器，可随时切换；另有 🌐 重译功能，把已有精灵的名字与图鉴描述迁移到当前语言。
